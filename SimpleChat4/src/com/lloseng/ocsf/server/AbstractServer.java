...
public abstract class AbstractServer implements Runnable {
	...
	final public boolean isListening() {
		return connectionListener != null && connectionListener.isAlive(); // modified
																			// in
																			// version
																			// 2.31
	}
	...
	public String getUserList() {
		...
		for (int i = 0; i < getNumberOfClients(); i++) {
			while (((ConnectionToClient)getClientConnections()[i]).getInfo("name") == null) {
			}
			clientList = clientList
					+ ((ConnectionToClient)getClientConnections()[i]).getInfo("name") + '\n';
		}
		return clientList;
	}
	...
	final public void run() {
		...
		try {
			...
			while (!readyToStop) {
				...
				/*if (clients != getNumberOfClients()) { // si le nombre de clients a change
					clients = getNumberOfClients(); // on met le nombre de clients a jour
					sendToAllClients("$" + getUserList()); // on update l'userList
				}*/
